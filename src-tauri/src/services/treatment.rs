...
impl TreatmentRecordService {
    pub async fn create(pool: &SqlitePool, input: CreateTreatmentRecordInput) -> AppResult<TreatmentRecord> {
        ...
        let treatment_record = sqlx::query_as::<_, TreatmentRecord>(
            "INSERT INTO treatment_records (id, visit_id, procedure_id, tooth_quadrant, quantity, procedure_price, treatment_notes, performed_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)
             RETURNING id, visit_id, procedure_id, tooth_quadrant, quantity, procedure_price, treatment_notes, performed_at"
        )
        .bind(&id)
        .bind(&input.visit_id)
    ...
    pub async fn list_for_visit(pool: &SqlitePool, visit_id: &str) -> AppResult<Vec<TreatmentRecord>> {
        let records = sqlx::query_as(
            "SELECT id, visit_id, procedure_id, tooth_quadrant, quantity, procedure_price, treatment_notes, performed_at FROM treatment_records WHERE visit_id = ? ORDER BY performed_at DESC"
        )
        .bind(visit_id)
        .fetch_all(pool)
        .await?;
